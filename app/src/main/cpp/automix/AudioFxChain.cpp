...
void AudioFxChain::process (float* const* channels, int numChannels, int numSamples)
{
    ...
    // при «всё выключено». Хронология (полевой репорт владельца, Honor): база
    // 29.06 (EQ/бас/лимитер) в дедлайн fast-path влезала, а вот июльский стек
    // FX (реверб — самый тяжёлый, + параметрик/сатурация) — уже нет; при
    // выключенных стадиях колбэк обязан быть бит-в-бит и по стоимости как до
    // FX-цепочки, страховка от включённого стека — sink-фолбэк по xrun'ам.
